...
$$
\alpha_{vu}^{(k)}=softmax(\mathrm{LeakyReLU(a^T[W^{(k)}h_v^{(k-1)}||W^{(k)}h_u^{(k-1)}])})
$$
여기서 $||$는 concat 연산자, 즉 행렬들을 나란히 합치는 것을 나타낸다. 이렇게 합쳐진 행렬에 대해 $a^T$를 곱해주는 것이다. 그리고 $a$가 바로 이 attention 의 학습 파라매터가 되며, GAT에서는 activation function 으로 LeakyReLU로 고정하여 정의했다는 것을 알 수 있다. 이를 통해 최종적으로 $\alpha_{vu}^{(k)}$는 $u$의 hidden state들에 대해서 $v$가 영향을 받는 정도에 대한 값을 갖는데, softmax를 사용했기 때문에 이 기여도의 총합은 항상 1이 된다. 
